...
function doubleLetters(letters){
  ...
  for(var i=0; i<letters.length; i++){
    double += letters[i]+letters[i];
  }
  return double;
  
}

console.log(doubleLetters(letters));
...
function doubleNumbers(array){
   ...
   */
   ...
   function multiplyNumbers(array){
    ...
    var times = array.map(function(num){
      return num*5;
    });
    
    for(i = 0; i < array.length && i < times.length; i++){
      multiplied[array[i]] = times[i]; 
    }
    return multiplied;
    }
...
// console.log(sumLetters([45, 246]))
   ...
   */
   ...
   console.log(letterMap(stringCount));
